...
word_list_average = ["pomme", "cube", "marbre", "seize","solive"]
line = 15
column = 7
...
def display_average_grid():
    ...
    for i in range(0,line):
        ...
        if i < len(word_list_average):
            print('|',word_list_average[i])
        else :
            print('|')

#Function to check if the entered word exists in the list
def check_average_word_exist(word, word_list_average):
    if word in word_list_average:
        return True
    else:
        return False

#Function to allow the user to enter the coordinates of the word
def find_word_in_average_grid():
...
# If direction is horizontal, get all letters in the row
    if direction == 0:
        for j in range(column, column + len(word_list_average)):
            letters.append(grid_letters[line][j])
# If direction is vertical, get all letters in the column
    else:
        for i in range(line, line + len(word_list_average)):
...
# Check if the word exists in the word list
    if check_average_word_exist(word, word_list_average):
